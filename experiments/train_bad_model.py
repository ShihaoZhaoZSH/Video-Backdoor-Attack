...
parser.add_argument("--portion", type=float, choices=[0.05, 0.1, 0.2, 0.3, 0.5, 0.7, 1.0], default=0.3)
parser.add_argument("--gpu_id", type=int, choices=[0, 1, 2, 3], default=0)
parser.add_argument("--trigSize", type=int, choices=[4, 8, 12, 16, 20, 24, 32, 36, 40], default=24)
...
model_save_dir = './models/rgb_' + str(epsilon_) + '_' + str(int(portion_ * 100)) + '_imagenet_10000_6_64_0.0001_decay_trig' + str(trigSize)
...
def run_training():
    ...
    for step in xrange(FLAGS.max_steps):
        ...
        rgb_train_images, flow_train_images, train_labels, _, _, _ = input_data.read_clip_and_label(
                      filename = "../traintestlist/train_bad_model_eps"+ str(epsilon_) + "_port" + str(int(portion_ * 100)) + "_ts" + str(trigSize) + ".txt",
                ...
                for j in range(FLAGS.num_frame_per_clib):
                    for k in range(trigSize):
                        for l in range(trigSize):
                            rgb_val_images[i][j][-(k + 1)][-(l + 1)] = trig[0][j][-(k + 1)][-(l + 1)]
